...
def create_group(group: schemas.GroupCreate, db: Session = Depends(get_db)):
    ...
    crud_bot.create_group(db=db, group=group)
    return JSONResponse(
...
@router.post("/{group_id}/assign-user/")
def assign_user_to_group(group_id: int, user_id: int, db: Session = Depends(get_db)):
    ...
    if user in group.users:
        return JSONResponse(
            ...
            content={"message": "User is already assigned to this group"},
        )
    ...
    return JSONResponse(
        ...
        content={"message": "User successfully assigned to group"},
    )


@router.post("/{group_id}/assign-bot/")
def assign_bot_to_group(group_id: int, bot_id: int, db: Session = Depends(get_db)):
    ...
    return JSONResponse(
        status_code=status.HTTP_200_OK,
        content={"message": "Bot successfully assigned to group"},
    )
